...
int runproc(char *id, int prio, int start, int end) {
	int ppid = getpid();
	int pid = fork();
	if (pid < 0) {
		...
		for (int i = 0; i < 9; i++) {
			...
			if (curr < start) {
				printf("FAIL: %s should not be running before time %d\n", id, start);
				exit(1);
			}
			if (curr > end) {
				printf("FAIL: %s should not be running after time %d\n", id, end);
				exit(1);
			}
			spin();
		}
...
int main() {
	...
	int childA = runproc("A", MAXPRIO-1, 0, 5);
	int childB = runproc("B", MAXPRIO-2, 4, 9);
	int childC = runproc("C", MAXPRIO-3, 8, 13);
	...
	printf("Process order should be: A ... B ... C\n");
